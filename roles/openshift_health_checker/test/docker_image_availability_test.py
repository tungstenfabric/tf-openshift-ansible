...
def test_is_available_skopeo_image():
    ...
    with patch.object(DockerImageAvailability, 'execute_module_with_retries') as m1:
        ...
        m1.assert_called_with('command', {'_uses_shell': True, '_raw_params': ' timeout 60 skopeo inspect --tls-verify=true  docker://registry.redhat.io/openshift3/ose-pod'})
        assert dia.is_available_skopeo_image('insecure.redhat.io/openshift3/ose-pod') is True
        m1.assert_called_with('command', {'_uses_shell': True, '_raw_params': ' timeout 60 skopeo inspect --tls-verify=false  docker://insecure.redhat.io/openshift3/ose-pod'})
    ...
    with patch.object(DockerImageAvailability, 'execute_module_with_retries') as m1:
        m1.return_value = result
        assert dia.is_available_skopeo_image('registry.redhat.io/openshift3/ose-pod') is True
        m1.assert_called_with('command', {'_uses_shell': True, '_raw_params': ' timeout 60 skopeo inspect --tls-verify=true --creds=test_user:test_pass docker://registry.redhat.io/openshift3/ose-pod'})
